...
async function nextAvailableSlots(tradie, startSearchDt, count = 3) {
  ...
  // ✅ FIX: ensure we never call .plus on null/invalid
    ...
    if (session.step === "confirm") {
      ...
      if (session.duplicateEvent && isUpdate) {
        ...
        session.lastPrompt = session.duplicateEvent
          ? "Sorry — say yes to keep both, update to replace the old booking, or no to change the time. (Press 1 yes, 2 no)"
          : "Sorry — just say yes to confirm, or no to change the time. (Press 1 yes, 2 no)";
        ask(twiml, session.lastPrompt, "/voice");
        return res.type("text/xml").send(twiml.toString());
      }
...
// ✅ DEPLOY SAFETY: crash early if file is truncated / PORT is weird
if (!PORT) throw new Error("PORT missing");

// NOTE: To catch truncation *before* running, set package.json start to:
