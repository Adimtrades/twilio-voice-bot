...
//
// ✅ DEPLOY FIXES INCLUDED (solves your Render "app is not defined" + port scan issues):
// - Ensures `const app = express()` is defined BEFORE any `app.post(...)`
// - Keeps a guaranteed `app.listen(PORT)` so Render detects an open port
// - Adds crash logging + unhandled rejection handlers for easier Render debugging
// - Keeps urlencoded body parser with rawBody capture (Twilio signature-safe)
//
// ✅ FEATURES INCLUDED (as per your block):
...
// ✅ MUST exist before any routes (fixes: "ReferenceError: app is not defined")
const app = express();
...
// Optional: crash hard so Render restarts (usually desirable)
process.exit(1);
});
...
// Twilio validates against the full URL of the webhook
...
/* ============================================================================
Health check (Render port scan needs an open port + route is nice)
============================================================================ */
...
// ✅ This is what stops Render from timing out on port scan
